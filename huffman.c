 ...
 *
 * Generates a modified linked list of nodes containing data(a character from the given text file) and 
 * frequency(the amount of times the said character(data) appears in the given text file).
 * 
 * The huffman tree will then be generated by removing the two nodes with the lowest frequencies, adding them to a binary tree 
 * as leaf nodes, with a parent node that contains data('*') and frequency(the sum of the two frequencies of the leaf nodes).
 * This process is repeated until the linked list contains only a root parent node.
 * 
...
void encode_c(node_l* node, char* encoding){
    //Encoding function, recursively moves through tree, adds to encoding if leaf node
    //Base case
	if (node==NULL) {
		return;
	}
